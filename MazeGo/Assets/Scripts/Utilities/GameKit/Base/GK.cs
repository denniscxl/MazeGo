...
namespace GKBase
{
    public class GK
    {
        ...
        /*==================================================*\
        | RotatingCalipers 旋转卡壳 O(N)
        | CALL: res = graham(pnt, n); res为最远点距离平方;
        \*==================================================*/
        public int RotatingCalipers(Point [] pnt)
        {
            int q = 1, ans = 0, i;
            int n = pnt.GetLength(0);
            for (i = 0; i < n; i++)
            {
                while ((pnt[(i + 1) % n] - pnt[i]) * (pnt[(q + 1) % n] - pnt[i]) > (pnt[(i + 1) % n] - pnt[i]) * (pnt[q] - pnt[i]))
                    q = (q + 1) % n;
                ans = Mathf.Max(ans, Mathf.Max((pnt[i] - pnt[q]).Len2(), (pnt[(i + 1) % n] - pnt[(q + 1) % n]).Len2()));
            }
            return ans;
        }
        ...
        /// <returns></returns>
        public static Tuple<Point, Point> RotatingCalipers(List<Point> points)
        {
            // 计算凸包
            ...
            // 初始化最远点对和距离
            double maxDist = 0;
            Tuple<Point, Point> maxPair = null;
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                while (true)
                {
                    int dx1 = hull[end].x - hull[start].x;
                    int dy1 = hull[end].y - hull[start].y;
                    int dx2 = hull[(end + 1) % n].x - hull[start].x;
                    int dy2 = hull[(end + 1) % n].y - hull[start].y;
                ...
                // 更新最远点对和距离
                double dist = Distance(hull[start], hull[end]);
                if (dist > maxDist)
                {
                    maxDist = dist;
                    maxPair = new Tuple<Point, Point>(hull[start], hull[end]);
                }
        ...
        public static int Orientation(Point p, Point q, Point r)
        {
            ...
            if (orientation == 0)
                return 0;  // Collinear
            return (orientation > 0) ? 1 : 2;  // Clockwise or Counterclockwise
        }
        ...
        public static List<Point> ConvexHull(List<Point> points)
        {
            ...
            // Find the point with the lowest Y coordinate (and lowest X coordinate in case of tie)
            int l = 0;
            for (int i = 1; i < n; i++)
            {
            ...
            Point lowest = points[l];
            points.RemoveAt(l);
            points.Sort((a, b) =>
            {
                int orientation = Orientation(lowest, a, b);
                if (orientation == 0)
                    return Distance(lowest, a).CompareTo(Distance(lowest, b));
                return (orientation == 2) ? -1 : 1;
            });

            // Add the lowest point back to the beginning of the sorted list
            ...
            for (int i = 2; i < n; i++)
            {
                while (hull.Count > 1 && Orientation(hull.ElementAt(hull.Count - 2), hull.Peek(), points[i]) != 2)
                {
                    hull.Pop();
                }
                hull.Push(points[i]);
