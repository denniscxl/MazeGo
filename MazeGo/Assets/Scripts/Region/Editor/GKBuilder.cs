...
public class GKBuilder
{
    ...
    static public void Building(RegionDefine.Channel channel, bool bLarger, bool bObfuscation, bool bDebug = false)
    {
        ...
        BuildOptions buildOptions = bDebug
            ? BuildOptions.SymlinkSources | BuildOptions.Development | BuildOptions.ConnectWithProfiler | BuildOptions.AllowDebugging
            : BuildOptions.None;
        ...
        //}
#else
        //var err = BuildPipeline.BuildPlayer(levels.ToArray(), targetPathName, EditorUserBuildSettings.activeBuildTarget, buildOptions);
        //if (err.files.Length == 0 && (File.Exists(targetPathName) || Directory.Exists(targetPathName))) {
        //    Debug.Log("Unity build success.（" + targetPathName + "）");
        //} else {
        //    Debug.LogError(string.Format("Unity build faliure！（{0}). err: {1}", targetPathName, err));
        //}
